...
export class LuckWheel extends Laya.Script {
    ...
    /** 当前选择的内转盘索引指定的分割数据 */
    public get currentInnerSplitData() { return this.outsideSplitDatas[this._innerSelectIndex]; }
    ...
    public startRotating(): void {
        ...
        switch (this.mode) {
            case LuckWheelMode.SingleRotatePointer:
                ...
                this._pointerRotationalObj.init(this._pointerAngle, this.initPointerRpm, this._rotateFriction);
                break;
            case LuckWheelMode.SingleFixedPointer:
                this._outsideRotationalObj.setRewardAngle(NaN);
                this._outsideRotationalObj.init(this.outsideDisc.rotation, this.initOutsideDiscRpm, this._rotateFriction);
                break;
            case LuckWheelMode.DoubleFixedPointer:
                this._outsideRotationalObj.setRewardAngle(NaN);
                this._outsideRotationalObj.init(this.outsideDisc.rotation, this.initOutsideDiscRpm, this._rotateFriction);
                ...
                this._innerRotationalObj.init(this.innerDisc.rotation, this.initInnerDiscRpm, this._rotateFriction);
                break;
        }
    }
    ...
    public setRewardIndex(outsideRewardIndex: number, innerRewardIndex?: number): void {
        ...
        switch (this.mode) {
            case LuckWheelMode.SingleRotatePointer:
                rewardAngle = this.getRewardAngleByIndex(this._outsideRewardIndex, outsideSplitData);
            ...
            case LuckWheelMode.DoubleFixedPointer:
                ...
                if (innerSplitData) {
                    const rewardAngle2 = this.getRewardAngleByIndex(this._innerRewardIndex, innerSplitData);
    ...
    public setRewardAngle(outsideRewardAngle: number, innerRewardAngle?: number): void {
        // 转为 [0, 360]
        outsideRewardAngle = Laya.MathUtil.repeat(outsideRewardAngle + this.currentOutsideSplitData.angleOffset, 360);
        // 根据奖励角获取外转盘奖励索引
        this._outsideRewardIndex = this.getOutsideIndexByAngle(outsideRewardAngle);

        if (!isNaN(innerRewardAngle)) {
            // 转为 [0, 360]
            innerRewardAngle = Laya.MathUtil.repeat(innerRewardAngle + this.currentInnerSplitData.angleOffset, 360);
            // 根据奖励角获取内转盘奖励索引
    ...
    private getSplitPositions(splitData: SplitData, radius: number, isAddCenter: boolean, out?: number[]): number[] {
        ...
        let { angleOffset, splitAngles } = splitData;
        ...
        for (let i = 0, len = splitAngles.length; i < len; i++) {
            ...
            let x = Math.cos(rad + angleOffset) * radius;
            let y = Math.sin(rad + angleOffset) * radius;
    ...
    private getRewardAngleByIndex(rewardIndex: number, splitData: SplitData): number {
        ...
        rewardAngle = Laya.MathUtil.repeat(rewardAngle + angleOffset, 360);
        return rewardAngle;
    }
...
class RotationalObject extends Laya.EventDispatcher {
    ...
    public static readonly ROTATE_END: string = "rotateEnd";
    /** 最小转速常量<正数> */
    private readonly MIN_RPM: number = 0.1;
    ...
    /** 到达目标转速的插值，区间为：[0,1]*/
    private _rpmInterpolation: number;
    /** 奖励角度 [0,360] */
    ...
    /** 到达目标转速的插值，区间为：[0,1]*/
    public get rpmInterpolation(): number { return this._rpmInterpolation; }
    /** 旋转启动后，慢慢加速到达的目标转速<度> */
    ...
    public init(angle: number, rpmTarget: number, rotateFriction: number = 0.985): void {
        ...
        this._rpmInterpolation = 0;
        this._rpm = rpmTarget * this._rpmInterpolation;
    ...
    public update(): void {
        ...
        if (isNaN(this._rewardAngle)) {
            // 慢慢旋转加速到达目标转速
            this._rpmInterpolation = Math.min(this._rpmInterpolation + 0.025, 1);
            this._rpm = Laya.MathUtil.lerp(0, this._rpmTarget, this._rpmInterpolation);
            if (Math.abs(this._rpm) < this.MIN_RPM) {
                // 限制最小转速
                this._rpm = Math.sign(this._rpm) * this.MIN_RPM;
            }
        ...
        if (this._isEasing) { // 缓动中...
            ...
            if (t >= 1) { // 当缓动的进度满时，由于小数计算的精度问题，当前角度并不一定就到达目标奖励角
                if (deltaAngle > this.MIN_RPM) { // 给一个最小转速，当与目标奖励角还有距离时，继续以最小转速旋转
                    this._rpm = Math.sign(this._rpm) * this.MIN_RPM;
                    this.setAngle(this._angle + this._rpm);
                } else { // 到达目标奖励角，结束旋转
